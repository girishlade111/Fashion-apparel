      ...
      <div className="grid md:grid-cols-2 gap-8 lg:gap-12">
        ...
        <div>
          ...
          <div className="flex items-center justify-between gap-4 mt-1">
            <h1 className="font-heading text-2xl lg:text-3xl text-neutral-900">{initial.name}</h1>
            <button
              onClick={() => toggleWishlist(initial.id)}
              className="shrink-0 w-10 h-10 flex items-center justify-center rounded-full border border-neutral-200 hover:border-neutral-900 transition-colors"
              aria-label={isWishlisted(initial.id) ? "Remove from wishlist" : "Add to wishlist"}
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill={isWishlisted(initial.id) ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={isWishlisted(initial.id) ? "text-red-500" : "text-neutral-700"}>
